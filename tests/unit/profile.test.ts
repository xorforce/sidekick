...
describe('Profile Selection in Build', () => {
  it('should load profile configuration when --profile flag is provided', () => {
    // This will test profile loading logic
    // Expected: profile config values override defaults
  });

  it('should use default profile when no --profile flag is provided', () => {
    // Expected: uses last-used profile or default
  });

  it('should validate profile exists before using it', () => {
    // Expected: error if profile doesn't exist
  });

  it('should merge profile values with command-line overrides', () => {
